...
def sort_list_files(list_files):
    no_id=[]
    exp={('audio',"wav","mp3","ogg","amr") : [], ('images','jpeg','png','jpg','svg'): [], ('documents','doc','docx','txt','pdf','xls','xlsx','pptx','ppt') :[], ('video','avi','mp4','mov','mkv') : [] , ('archives','zip','gz','tar') : []}
...
def unpack(arc):
    if any(arc.iterdir()):
         ...
         for file in archive_list[0]:
            try:
                 arch_path=str(pathlib.Path(file).parent)+'/'+pathlib.PurePath(file).stem
